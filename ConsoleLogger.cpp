//Стороняя библеотека https://www.codeproject.com/Articles/13368/Multiple-consoles-for-a-single-application
							...
							const char	*helper_executable/*=NULL*/)
{
	...
	if (m_hPipe==INVALID_HANDLE_VALUE)
	{	// failure
		MessageBox(NULL,(LPCWSTR)"CreateNamedPipe failed",(LPCWSTR)"ConsoleLogger failed",MB_OK);
		return -1;
	}
	...
	wchar_t *cmdline=(wchar_t*)_Malloc((sizeof(wchar_t)*MAX_PATH)); //[MAX_PATH];;
	if (!helper_executable)
		helper_executable=DEFAULT_HELPER_EXE;

	sprintf((char*)cmdline,"%s %s",helper_executable,logger_name);
	...
	{	// on failure - try to get the path from the environment
		...
		if (path)
		{
			sprintf((char*)cmdline,"%s %s",path,logger_name);
			bRet = CreateProcess(NULL,cmdline,NULL,NULL,FALSE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi);
		}
		if (!bRet)
		{
			MessageBox(NULL,(LPWSTR)"Helper executable not found",(LPWSTR)"ConsoleLogger failed",MB_OK);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
		}
	...
	if (!bConnected)
	{
		MessageBox(NULL,(LPWSTR)"ConnectNamedPipe failed",(LPWSTR)"ConsoleLogger failed",MB_OK);
	...
	{
		MessageBox(NULL,(LPWSTR)"WriteFile failed(1)",(LPWSTR)"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
	...
	{	// Send buffer-size
		...
		if (cbWritten!=strlen(buffer))
		{
			MessageBox(NULL,(LPWSTR)"WriteFile failed(2)",(LPWSTR)"ConsoleLogger failed",MB_OK);
			DisconnectNamedPipe(m_hPipe);
			CloseHandle(m_hPipe);
			m_hPipe=INVALID_HANDLE_VALUE;
			return -1;
	...
	if (cbWritten!=1)
	{
		MessageBox(NULL,(LPWSTR)"WriteFile failed(3)",(LPWSTR)"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
